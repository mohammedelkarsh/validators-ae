...
final class UaeMobile
{
    ...
    public function validate(mixed $value): ValidationResult
    {
        ...
        if ($normalized === '') {
            return ValidationResult::invalid('', 'ae.mobile.required');
        }

        if (! preg_match(self::LOCAL_PATTERN, $normalized)) {
            return ValidationResult::invalid($normalized, 'ae.mobile.invalid_format');
        }

        return ValidationResult::valid($normalized, [
            'international' => '+971'.substr($normalized, 1),
        ]);
    }
    ...
    public static function normalize(mixed $value): string
    {
        ...
        if (str_starts_with($input, '+971')) {
            $input = '0'.substr($input, 4);
        } elseif (str_starts_with($input, '971')) {
